...
ChangeCheckConflictsSNC.CONFLICT_CHECK_COMPLETE = gs.getMessage("The conflict check is complete");
...
ChangeCheckConflictsSNC.allowConflictDetection = function(currentGr, previousGr, config) {
	...
	var hasItem = currentGr.cmdb_ci && !currentGr.cmdb_ci.nil();
...
ChangeCheckConflictsSNC.prototype = {
	...
	initialize: function(sourceRecord, config) {
		...
		this.blackouts = [];
		if (config.blackout)
			this.blackouts = config.blackout;
		else if (this.bCheckConflictBlackout)
			this.blackouts = ChangeCollisionHelper.getBlackoutsByDate(this.startDate, this.endDate, this.sourceRecord);

		this.maintenanceSchedules = [];
		if (config.maintenance)
			this.maintenanceSchedules = config.maintenance;
		else if (this.bCheckChangeConflictCurrentWindow || this.bCheckConflictRelatedChildWindow || this.bChangeConflictRelatedParentWindow || this.bCheckConflictRelatedServices)
			this.maintenanceSchedules = ChangeCollisionHelper.getConditionalMaintenanceSchedules(this.sourceRecord);
	...
	*/
	...
	_checkIterating: function() {
		...
		tracker.updateDetailMessage(ChangeCheckConflictsSNC.CONFLICT_CHECK_COMPLETE);
		tracker.success(ChangeCheckConflictsSNC.CONFLICT_CHECK_COMPLETE);
	...
	_checkComposed: function() {
		...
		// If the tracker is cancelled we should cease detecting conflicts
		if (tracker.isCancelled())
			return;
		...
		if (this.bCheckChangeConflictCurrentCI) {
			// If the tracker is cancelled we should stop checking conflicts
			if (!this._increaseProgress(gs.getMessage("Checking already scheduled CIs")))
				return;
		...
		if (this.bChangeConflictCIMaintSchedule) {
			if (!this._increaseProgress(gs.getMessage("Checking attached maintenance schedules")))
				return;
		...
		if (this.bCheckConflictAssignedTo) {
			if (!this._increaseProgress(gs.getMessage("Checking already assigned change requests")))
				return;
			this.checkAssignedToAlreadyScheduled(ChangeConflict.CHANGETYPE_ASSIGNED_TO_ALREADY_SCHEDULED);
		}

		if (!this._increaseProgress(gs.getMessage("Saving conflicts data")))
			return;
		...
		this.tracker.updateDetailMessage(ChangeCheckConflictsSNC.CONFLICT_CHECK_COMPLETE);
		this.tracker.success(ChangeCheckConflictsSNC.CONFLICT_CHECK_COMPLETE);
	...
	*/
	checkAndUpdate: function() {
		...
		// Conflict fields should not be updated if the tracker was cancelled
		if (this.tracker && this.tracker.isCancelled())
			return;

		var conflictLastRun = new GlideDateTime();
        var conflictStatus = nSavedConflictCount ? "Conflict" : "No Conflict";

		// Ensure we have an up to date version of the record to update
		var recToUpdate = new GlideRecord(this.sourceRecord.getRecordClassName());
	...
	stageD: function(ciGR) {
		if (this.USE_DEPRECATED_DEPENDENCY_FUNC) {
			...
			var dependenciesGR = ChangeCollisionHelper.getCIDependencies(ciGR.sys_id, this.sourceRecord);
	...
	stageE: function(ciGR) {
		if (this.USE_DEPRECATED_DEPENDENT_FUNC) {
			...
			var dependantsGR = ChangeCollisionHelper.getCIDependants(ciGR.sys_id, this.sourceRecord);
	...
	*/
	checkAlreadyScheduledComposed: function(processedCIs, dependencyTypes, conflictType) {
		...
		this._dependencyQuery('change_request', 'cmdb_ci', processedCIs, dependencyTypes, (function(gr) {
			...
			gr.addActiveQuery();
			gr.addDomainQuery(this.sourceRecord);
	...
	*/
	_addConflict: function(ciSysId, conflictType, conflictingElementId, relatedCIId) {
		...
		switch (conflictType) {
			...
			case ChangeConflict.CHANGETYPE_PARENT_ALREADY_SCHEDULED:
				...
				this._addConflictingChange(conflictingElementId, relatedCIId, conflictType === ChangeConflict.CHANGETYPE_PARENT_ALREADY_SCHEDULED ? ChangeConflict.CHANGETYPE_CHILD_ALREADY_SCHEDULED : ChangeConflict.CHANGETYPE_PARENT_ALREADY_SCHEDULED, ciSysId, impactedService);
				break;
	...
	updateConflictDetectionFields: function(conflictLastRun) {
		...
		changeGr.addDomainQuery(this.sourceRecord);
		...
		// DO NOT change this code to remove the loop and use updateMultiple.
		// addDomainQuery has a side effect where it also SETS the domain value field
		// which would then update the domain on conflicting Change we're updating!
		while (changeGr.next()) {
	...
	*/
	_getChangesWithCIs: function(ciId) {
		...
		var affectedCisConflicts = ChangeCollisionHelper.getChangesWithAffectedCi(ciId, this.startDate, this.endDate, this.sourceRecord);
	...
	*/
	_dependencyQuery: function(table, key, dependants, dependencyTypes, precall, callback) {
		...
		if (this.bCheckConflictRelatedServices) {
			...
			this._queryRelated(table, key, dependants, 'svc_ci_assoc', 'service_id', 'ci_id', precall, callback.bind(this, ChangeCheckConflictsSNC.CHILDREN));
		}
	},
...
ConflictExecutionTracker.prototype = {
	...
	// We need to query to see if it has cancelled (GlideExecutionTracker.isCancelled() is not provided unfortunately...)
